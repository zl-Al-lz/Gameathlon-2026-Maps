...
extern const int modBiasModeratelyAggressive = (1 << 45); // Reserve high-order bits for custom/mod biases to reduce the chance
// of conflicts if the base library adds new official bias flags.

// This function can be overwritten to add additional bias types!
mutable void addCustomSectorBiases(int arID = cInvalidID, int biasFlags = cBiasNone)
{
   SectorCircle sc;
   if((biasFlags & modBiasModeratelyAggressive) != 0)
   {
      sc.set(-0.375 * cPi, 0.375 * cPi);
      addSectorBiasToRandomizer(arID, sc);
   }
}
...
void lightingOverride()
{
   ...
   rmTriggerAddScriptLine("   trSetLighting(\"biome_greek_temperate_dusk_03_mod\",0.0);");
...
void generate()
{
   ...
   float placementFraction = gameIs1v1() ? 0.24 : (0.245 + (0.0025 * cNumberPlayers));
   float placementAngle = randRadian();

   // Override the angle in 1v1.
   if(gameIs1v1())
      placementAngle = (xsRandBool(0.5) == true) ? cPi : cPiOver2;
   ...
   if(gameIs1v1() == true)
   {
      int firstSettlementBias = 0;
      int secondSettlementBias = 0;

      bool biasType = xsRandBool(0.5);

      if(sharedSide == cLocSideSame)
      {
         firstSettlementBias = cBiasBackward | cBiasDefensive;
         secondSettlementBias = cBiasAggressive | cBiasForward;
      }
      else
      {
         firstSettlementBias = biasType ? modBiasModeratelyAggressive : cBiasBackward | cBiasDefensive;
         secondSettlementBias = biasType ? cBiasDefensive : cBiasAggressive | cBiasForward;
      }

      addSimObjectLocsPerPlayerPair(firstSettlementID, false, 1, 55.0, 75.0, cSettlementDist1v1, firstSettlementBias, 
                                    cInAreaDefault, sharedSide);
      addSimObjectLocsPerPlayerPair(secondSettlementID, false, 1, 75.0, 100.0, cSettlementDist1v1, secondSettlementBias, 
                                    cInAreaDefault, sharedSide);
   }
   else
   {
   ...
   // Starting objects use the default radial and angular variation values.
   float simLocStartingObjectsRadiusVar = cSimLocDefaultRadiusVar;
   float simLocStartingObjectsAngleVar = cSimLocDefaultAngleVar;
   ...
   if(gameIs1v1())
   {
      ...
      int[] startingGoldLocIDs = addMirroredLocsPerPlayerPair(1, cStartingGoldMinDist, cStartingGoldMaxDist, cStartingObjectAvoidanceMeters);

      // Apply radial and angular variation.
      setLocsRadiusVariance(startingGoldLocIDs, simLocStartingObjectsRadiusVar);
      setLocsAngleVariance(startingGoldLocIDs, simLocStartingObjectsAngleVar);

      // Place objects at the distorted locations.
      setLocsObject(startingGoldLocIDs, startingGoldID, false);
   }  
   else
   {
   ...
   {
      rmObjectDefAddItem(startingHerdID, (xsRandBool(0.5) == true) ? cUnitTypeGoat : cUnitTypeCow, xsRandInt(2, 3));
   }
   ...
   if(gameIs1v1() == true)
   {
      addSimObjectLocsPerPlayerPair(closeGoldID, false, 1, 50.0, 70.0, avoidGoldMeters, cBiasVeryDefensive, cInAreaDefault, sharedSide);
   ...
   if(gameIs1v1())
   {
      addSimObjectLocsPerPlayerPair(bonusGoldID, false, 3 * getMapAreaSizeFactor(), 70.0, -1.0, avoidGoldMeters, cBiasNone, 
                                    cInAreaDefault, sharedSide);
   }
   else
   {
   ...
   if(gameIs1v1() == true)
   {
      addSimObjectLocsPerPlayerPair(closeHuntID, false, 1, 55.0, 80.0, avoidHuntMeters, cBiasNone, cInAreaDefault, sharedSide);
   }
   else
   {
   ...
   if(gameIs1v1() == true)
   {
      addSimObjectLocsPerPlayerPair(bonusHuntID, false, 1, 80.0, 120.0, avoidHuntMeters, cBiasNone, cInAreaDefault, sharedSide);
   }
   else
   {
   ...
   if(gameIs1v1() == true)
   {
      addSimObjectLocsPerPlayerPair(farHunt1ID, false, 1, 70.0, 100.0, avoidHuntMeters, cBiasNone, cInAreaDefault, sharedSide);
   }
   else
   {
   ...
   {
      rmObjectDefAddItem(closeHerdID, (xsRandBool(0.5) == true) ? cUnitTypeGoat : cUnitTypeCow, 2);
   }
   ...
   {
      rmObjectDefAddItem(bonusHerdID, (xsRandBool(0.5) == true) ? cUnitTypeGoat : cUnitTypeCow, xsRandInt(2, 3));
   }
   ...
   int fishAvoidance = rmCreateTypeDistanceConstraint(cUnitTypeFishResource, fishDistMeters);
   int fishAvoidEdge = createSymmetricBoxConstraint(rmXTileIndexToFraction(4), rmXTileIndexToFraction(4));
   int fishAvoidLand = rmCreatePassabilityDistanceConstraint(cPassabilityLand, true, 8.0);
   ...
   else
   {
      for(int i = 0; i < 4; i++)
      {
         ...
         rmObjectDefAddConstraint(fishID, fishAvoidance);
   ...
   {  
      ...
      int plantsDensity= 24;
      int plantsGroupDensity = 3;
      switch(i)
      {
         // Plants.
